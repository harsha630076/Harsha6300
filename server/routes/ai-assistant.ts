...
const getResponseCategory = (message: string): keyof typeof mockResponses => {
  ...
  if (lowerMessage.includes('tired') || lowerMessage.includes('energy') || lowerMessage.includes('mood') || lowerMessage.includes('stress')) {
    return 'mood';
  }
  
  if (lowerMessage.includes('calories') || lowerMessage.includes('protein') || lowerMessage.includes('nutrition') || lowerMessage.includes('eat')) {
    return 'nutrition';
  }
...
export const handleAIAssistant: RequestHandler = (req, res) => {
  try {
    ...
    if (context?.mood === 'low-energy') {
      suggestions.push("Try iron-rich foods", "Get some sunlight", "Consider a short walk");
    ...
    if (context?.dailyCalories && context.dailyCalories < 1500) {
      suggestions.push("Add healthy snacks", "Include more protein", "Try nuts or seeds");
      recommendations.push({
        type: 'food',
    ...
    recommendations.push({
      type: 'tip',
      content: 'Eating regularly throughout the day helps maintain stable energy levels'
    });
