...
export const handleCreateUser: RequestHandler = (req, res) => {
  try {
    ...
    NotificationService.create({
      ...
      message: 'Start your health journey by scanning your first meal with our AI camera feature.',
      action_url: '/scan'
    });
    ...
    NotificationService.create({
      ...
      message: 'Staying hydrated is key to maintaining energy levels. Aim for 8-10 glasses of water daily!',
  ...
  try {
    ...
    if (userLogs.length === 1) {
      NotificationService.create({
        ...
        message: 'Great start! You\'ve logged your first meal. Keep it up to build healthy habits.',
        action_url: '/insights'
      });
      ...
      NotificationService.create({
        ...
        type: 'achievement',
        title: '🔥 Week Streak!',
        message: 'Amazing! You\'ve logged meals for a week. You\'re building great habits!',
        action_url: '/insights'
      });
    }
...
export const handleExportUserData: RequestHandler = (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=quickcal-ai-data-${userId}.json`);
    res.json(exportData);
  } catch (error) {
